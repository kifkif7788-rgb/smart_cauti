...
## QR ที่เป็นรหัสเตียงโดยตรง

ตัวสแกนในแอปรองรับข้อความ `SM-B02` (รวมตัวพิมพ์เล็ก/ช่องว่างหัวท้าย) และส่งไป `/assess/SM-B02` เหมือนกรอกรหัสเอง ต้อง login ก่อน และ server ตรวจว่ามีป้าย/ผู้ป่วยหรือพาไปลงทะเบียนเตียงว่าง
QR แบบ URL `/s/SM-B02?k=...` ยังคงเข้าทางตรวจ HMAC หากลายเซ็นเสียหรือหายจะไม่เปลี่ยนไปใช้ทางกรอกรหัสเอง
ตัวแปลงอยู่ที่ `src/lib/qr-input.ts` และใช้บน browser ได้โดยไม่ import `node:crypto`
QR ข้อความล้วนต้องสแกนจากภายในแอป หากต้องการให้แอปกล้องโทรศัพท์เปิดเว็บโดยตรง ให้ใช้ QR แบบ URL ที่พิมพ์จาก `/admin/tags`

## ปุ่มจัดการผู้ป่วย

`EpisodeActions` แสดง 3 ปุ่มข้อความ ไม่มีไอคอน: ย้ายเตียง / ถอดสาย / จำหน่าย
ย้ายเตียงใช้ API transfer เดิม ถอดสายและจำหน่ายเปิดฟอร์มแยกหัวข้อ วันที่ และข้อความยืนยัน โดยใช้ API close เดิม
กลุ่มถอดสาย: หมดข้อบ่งชี้, เปลี่ยนสายใหม่, อื่น ๆ; กลุ่มจำหน่าย: จำหน่ายผู้ป่วย, ย้ายหอผู้ป่วย, ผู้ป่วยเสียชีวิต
เมื่อเปลี่ยนฟอร์มจะล้างเหตุผลเดิมเพื่อไม่ให้บันทึกผิดประเภท ต้องกดยืนยันจึงปิดรายการ

## เมนูสร้าง QR สำหรับ Admin

`AppHeader` เป็น async Server Component อ่าน session ฝั่งเซิร์ฟเวอร์ และแสดง “สร้าง QR” เหนือ “สแกน QR” เฉพาะ role `ADMIN` ลิงก์ไป `/admin/tags` สำหรับสร้างและพิมพ์ป้ายประจำเตียง
หน้า `/admin/tags` ยังคงตรวจสิทธิ์ฝั่ง server ตามเดิม

## หน้าป้าย QR

`src/app/admin/tags/page.tsx` ใช้ AppHeader และ ShieldMark ร่วมกับธีมหลัก มีแบนเนอร์ฟ้า–เขียว สรุปจำนวนป้าย คำแนะนำการพิมพ์ และ empty state เมื่อไม่มีป้าย
Style แยกใน `src/app/admin/tags/tags.module.css` เพื่อไม่ให้กระทบหน้าประเมิน การ์ดป้ายใช้พื้นขาวและ QR ดำคงที่แม้เปิด dark mode
Grid บนจอใหญ่ 4 คอลัมน์ แท็บเล็ต 3 มือถือ 2 (จอเล็กกว่า 360px เป็น 1) รูปแบบพิมพ์ A4 ขอบ 10mm, 3 คอลัมน์, ป้ายสูง 82mm, QR 38mm, ซ่อน header และส่วนควบคุม
QR เว้น margin 4 modules และใช้ลายเซ็นเดิม ควรตรวจ Print Preview และทดสอบสแกนป้ายจริงก่อนพิมพ์จำนวนมาก

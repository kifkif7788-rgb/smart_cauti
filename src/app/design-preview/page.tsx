...
export default async function DesignPreview({ searchParams }: { searchParams: Promise<{ screen?: string }> }) {
  ...
  const rows: AssessmentRow[] = episodes.map((e,i)=>({assessment_id:e.episode_id,client_uuid:e.episode_id,episode_id:e.episode_id,assessor_id:'preview',source:'NURSE',study_mode:'INTERVENTION',assessed_at:'2026-09-15T00:00:00Z',shift:'MORNING',foley_day:2,need:i!==0,fix:i!==1,flow:i!==2,below:true,closed:true,all_pass:i>2,feedback:i===0?'REVIEW_REMOVAL':i<3?'CORRECT_NOW':'PASS',notes:null,created_at:'2026-09-15T00:00:00Z'}));
  return <>
    ...
    {screen === 'check' && <Check5Form preview episode={{episodeId:'design-preview',hn:'DEMO-001',studyCode:'DEMO-001',bedNo:'3',wardCode:'ศัลยกรรมชาย',insertDate:'2026-09-13',insertDateTh:'13 ก.ย. 2569',foleyDay:3}} studyMode="INTERVENTION" assessedThisShift={false}/>}
    ...
    {screen === 'dashboard' && <DashboardView wardCodes={['หอผู้ป่วยตัวอย่าง']} list={episodes} rows={rows} pass={9} correct={2} review={1} percent={75} colors={['#2ebd87','#389df4','#ffbe2b','#9770e4','#ef7e9f']} trend={[10,11,12,12,11,12,12].map((count,i)=>({date:`2026-09-${9+i}`,count}))} peak={12} longStay={[]} historyError={false}/>}
  </>;
}

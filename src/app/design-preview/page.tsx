...
export default async function DesignPreview({ searchParams }: { searchParams: Promise<{ screen?: string }> }) {
  ...
  return <>
    ...
    {screen === 'dashboard' && <DashboardView period="day" passTrend={[{label:'เวรเช้า',total:4,passed:3,percent:75},{label:'เวรบ่าย',total:5,passed:4,percent:80},{label:'เวรดึก',total:0,passed:0,percent:null}]} wardCodes={['หอผู้ป่วยตัวอย่าง']} list={episodes} rows={rows} pass={9} correct={2} review={1} percent={75} colors={['#2ebd87','#389df4','#ffbe2b','#9770e4','#ef7e9f']} trend={[10,11,12,12,11,12,12].map((count,i)=>({date:`2026-09-${9+i}`,count}))} peak={12} longStay={[]} historyError={false}/>}
  </>;
}

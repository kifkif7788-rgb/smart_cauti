...
export default async function DesignPreview({ searchParams }: { searchParams: Promise<{ screen?: string }> }) {
  ...
  const rows: AssessmentRow[] = episodes.map((e,i)=>({assessment_id:e.episode_id,client_uuid:e.episode_id,episode_id:e.episode_id,assessor_id:'preview',source:'NURSE',nurse_level:i%2===0?'RN':'NA',study_mode:'INTERVENTION',assessed_at:'2026-09-15T00:00:00Z',shift:'MORNING',foley_day:2,need:i!==0,fix:i!==1,flow:i!==2,below:true,closed:true,all_pass:i>2,feedback:i===0?'REVIEW_REMOVAL':i<3?'CORRECT_NOW':'PASS',notes:null,created_at:'2026-09-15T00:00:00Z'}));
  return <>

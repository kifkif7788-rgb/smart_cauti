...
export function ScanHero({ nurseLevel, canChooseLevel = false }: { nurseLevel?: NurseLevel | null; canChooseLevel?: boolean }) {
  ...
  const features: { icon: IconName; label: string; href: string }[] = [{icon:'need',label:'เช็กทุกวัน',href:'/beds'},{icon:'chart',label:'ใช้ข้อมูลทันที',href:'/dashboard'},{icon:'bell',label:'ติดตามความเสี่ยง',href:'/risk'},{icon:'shield',label:'ปลอดภัย ลด CAUTI',href:'/learn'}];

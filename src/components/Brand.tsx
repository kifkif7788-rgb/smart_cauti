...
export function ScanHero({ showDashboard, nurseLevel }: { showDashboard: boolean; nurseLevel?: NurseLevel | null }) {
  ...
  const features: { icon: IconName; label: string; href: string }[] = [{icon:'need',label:'เช็กทุกวัน',href:'/scan'},{icon:'chart',label:'ใช้ข้อมูลทันที',href:dataHref},{icon:'bell',label:'ติดตามความเสี่ยง',href:'/risk'},{icon:'shield',label:'ปลอดภัย ลด CAUTI',href:'/learn'}];

...
import { NurseLevelSwitch } from './NurseLevelSwitch';
import type { NurseLevel } from '@/lib/check5';
...
export function ScanHero({ showDashboard, nurseLevel, canChooseLevel = false }: { showDashboard: boolean; nurseLevel?: NurseLevel | null; canChooseLevel?: boolean }) {
  ...
  // ยังไม่เลือกคุณวุฒิ = ยังสแกนไม่ได้ เพราะทุกการประเมินต้องรู้ว่าใครเป็นผู้ประเมิน
  // บัญชีที่เลือกไม่ได้ (คุณวุฒิมาจากบัญชี หรือไม่ใช่บุคลากรพยาบาล) ไม่ต้องรออะไร
  const locked = canChooseLevel && nurseLevel == null;
  const features: { icon: IconName; label: string; href: string }[] = [{icon:'need',label:'เช็กทุกวัน',href:'/beds'},{icon:'chart',label:'ใช้ข้อมูลทันที',href:dataHref},{icon:'bell',label:'ติดตามความเสี่ยง',href:'/risk'},{icon:'shield',label:'ปลอดภัย ลด CAUTI',href:'/learn'}];
  const QR = <><div className="scan-code"><svg viewBox="0 0 100 100" aria-hidden="true"><path d="M3 25V3h22m50 0h22v22M3 75v22h22m50 0h22V75" fill="none" stroke="#18ae7f" strokeWidth="2.5"/><g fill="#15342f"><path d="M13 13h25v25H13zm49 0h25v25H62zM13 62h25v25H13z"/><path d="M44 13h6v12h6v6H44zm0 25h12v6H44zm18 6h6v12h-6zm12 0h13v6H74zM13 44h12v6H13zm18 0h7v12H25v-6h6zm13 6h12v12H44zm18 12h12v6H62zm18-6h7v12h-7zm-36 12h6v19h-6zm12 6h12v13H56zm18 0h13v6H74zm7 7h6v6h-6zM13 53h6v5h-6z"/></g><g fill="white"><path d="M18 18h15v15H18zm49 0h15v15H67zM18 67h15v15H18z"/></g><path d="M22 22h7v7h-7zm49 0h7v7h-7zM22 71h7v7h-7z" fill="#15342f"/></svg><span className="scan-camera"><UiIcon name="arrow"/></span></div><span className="scan-label">SCAN ME</span></>;
  return <section className="scan-hero"><Brand/>{canChooseLevel && <NurseLevelSwitch initial={nurseLevel ?? null}/>}<h2>สแกน QR เพื่อเริ่มทำแบบประเมิน</h2><div className="scan-stage"><HospitalScene/>{locked ? <div className="scan-launch is-locked" aria-disabled="true">{QR}</div> : <Link href="/scan" className="scan-launch" aria-label="เปิดกล้องสแกน QR ที่เตียงผู้ป่วย">{QR}</Link>}</div><p className="scan-help">{locked ? 'เลือก RN หรือ NA ด้านบนก่อน จึงจะสแกน QR ได้' : 'แตะเพื่อเปิดกล้องสแกน QR ที่เตียงผู้ป่วย'}</p><div className="feature-row">{features.map(({icon,label,href},i)=><Link key={label} href={href}><span data-tone={i}><UiIcon name={icon}/></span><small>{label}</small></Link>)}</div></section>;
}

...
export function ScanHero({ showDashboard, nurseLevel }: { showDashboard: boolean; nurseLevel?: NurseLevel | null }) {
  ...
  return <section className="scan-hero"><Brand/>{nurseLevel !== undefined && <NurseLevelSwitch initial={nurseLevel}/>}<h2>สแกน QR เพื่อเริ่มทำแบบประเมิน</h2><div className="scan-stage"><HospitalScene/>{locked ? <div className="scan-launch is-locked" aria-disabled="true">{QR}</div> : <Link href="/scan" className="scan-launch" aria-label="เปิดกล้องสแกน QR ที่เตียงผู้ป่วย">{QR}</Link>}</div><p className="scan-help">{locked ? 'เลือก RN หรือ PN ด้านบนก่อน จึงจะสแกน QR ได้' : 'แตะเพื่อเปิดกล้องสแกน QR ที่เตียงผู้ป่วย'}</p><div className="feature-row">{features.map(({icon,label,href},i)=><Link key={label} href={href}><span data-tone={i}><UiIcon name={icon}/></span><small>{label}</small></Link>)}</div></section>;
}
